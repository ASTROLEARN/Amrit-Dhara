...
export function DataVisualization({ refreshKey }: DataVisualizationProps) {
   ...
   const [benchmarkType, setBenchmarkType] = useState<'WHO' | 'EPA' | 'Custom'>(
      'WHO'
   );
   ...
   const calculateOverviewMetrics = () => {
      ...
      const avgHpi =
         samples.reduce((sum, s) => sum + (s.hpi || 0), 0) / totalSamples;
      const highPollutionCount = samples.filter(
         (s) => (s.hpi || 0) > 100
      ).length;
      ...
      const firstHalfAvg =
         samples.slice(0, midPoint).reduce((sum, s) => sum + (s.hpi || 0), 0) /
         midPoint;
      const secondHalfAvg =
         samples.slice(midPoint).reduce((sum, s) => sum + (s.hpi || 0), 0) /
         (totalSamples - midPoint);
      ...
      setOverviewMetrics({
         ...
         complianceRate: (
            ((totalSamples - highPollutionCount) / totalSamples) *
            100
         ).toFixed(1),
      });
   };
   ...
   const getMostPollutedMetal = () => {
      ...
      const metalAverages = metals.map((metal) => {
         const avg =
            samples.reduce((sum, s) => sum + (s[metal] || 0), 0) /
            samples.length;
      ...
      return metalAverages.reduce((max, current) =>
   ...
   const getConcentrationData = () => {
      ...
      return metals.map((metal) => {
         const values = samples.map((sample) => {
            switch (metal) {
               case 'As':
                  return sample.arsenic;
               case 'Cd':
                  return sample.cadmium;
               case 'Cr':
                  return sample.chromium;
               case 'Pb':
                  return sample.lead;
               case 'Hg':
                  return sample.mercury;
               case 'Ni':
                  return sample.nickel;
               case 'Cu':
                  return sample.copper;
               case 'Zn':
                  return sample.zinc;
               default:
                  return 0;
   ...
   const getQualityDistribution = (index: string) => {
      ...
      return Object.entries(distribution).map(([name, value]) => ({
         ...
         value,
         color:
            name.includes('Clean') || name.includes('Low')
               ? COLORS.green
               : name.includes('Moderate') ||
                 name.includes('Medium') ||
                 name.includes('Slight')
               ? COLORS.yellow
               : name.includes('High') || name.includes('Severe')
               ? COLORS.red
               : COLORS.blue,
      }));
   };
   ...
   const getUniqueLocations = () => {
      const locationMap = new Map<
         string,
         { count: number; location: string; samples: Sample[] }
      >();
   ...
   const fetchLocationTrendData = async (locationName: string) => {
      ...
      try {
         ...
         const response = await fetch(
            `/api/results?location=${encodeURIComponent(
               locationName
            )}&limit=100`
         );
         const result = await response.json();
   ...
   const getCriticalBenchmarkLines = (
      selectedItems: string[],
      itemType: 'metals' | 'indices'
   ) => {
         ...
         selectedItems.forEach((metal, index) => {
            ...
            if (benchmarkValue > 0) {
               const maxDataValue = Math.max(
                  ...getFilteredTrendData().map((d) => d[metal] || 0)
               );
   ...
   const exportLocationTrendsToPdf = async () => {
      ...
      try {
         ...
         pdf.text('Location Trends Analysis Report', pageWidth / 2, 25, {
            align: 'center',
         });
         ...
         if (locationTrendData.length > 0) {
            const firstDate = new Date(
               locationTrendData[0].createdAt
            ).toLocaleDateString();
            const lastDate = new Date(
               locationTrendData[locationTrendData.length - 1].createdAt
            ).toLocaleDateString();
            pdf.text(
         ...
         const metalChartElement = document.getElementById(
         ...
         if (metalChartElement) {
            try {
               ...
               pdf.text(
                  `Selected Metals: ${selectedMetals
               ...
               const metalImgHeight =
                  (metalCanvas.height * metalImgWidth) / metalCanvas.width;
         ...
         const indexChartElement = document.getElementById(
         ...
         if (indexChartElement) {
            try {
               ...
               pdf.setFont('helvetica', 'bold');
               pdf.setTextColor(255, 255, 255); // White text
               pdf.text('Pollution Indices Trends', pageWidth / 2, yPosition, {
                  align: 'center',
               });
               pdf.setTextColor(0, 0, 0); // Reset to black
               yPosition += 15;
               ...
               const indexImgHeight =
                  (indexCanvas.height * indexImgWidth) / indexCanvas.width;
         ...
         pdf.setFont('helvetica', 'bold');
         pdf.setTextColor(255, 255, 255); // White text
         pdf.text('Summary Statistics & Analysis', pageWidth / 2, yPosition, {
            align: 'center',
         });
         pdf.setTextColor(0, 0, 0); // Reset to black
         yPosition += 20;
         ...
         const hpiCategory =
            avgHPI < 100 ? 'Low' : avgHPI < 200 ? 'Medium' : 'High';
         const hpiColor =
            avgHPI < 100
               ? [34, 197, 94]
               : avgHPI < 200
         ...
         const trend =
            hpiValues.length > 1
               ? hpiValues[hpiValues.length - 1] > hpiValues[0]
                  ? 'Increasing'
                  : hpiValues[hpiValues.length - 1] < hpiValues[0]
                  ? 'Decreasing'
                  : 'Stable'
         ...
         metals.forEach((metal) => {
            ...
            pdf.text(
               ...
               }: Avg ${avg.toFixed(4)} mg/L, Max ${max.toFixed(4)} mg/L`,
               20,
               yPosition
            );
            yPosition += 8;
         ...
         pdf.rect(
            ...
            yPosition - 11,
            pageWidth - 20,
            8 + Math.min(10, locationTrendData.length) * 7
         );
         ...
         pdf.setFont('helvetica', 'normal');
         pdf.text(
            `Generated by Amrit Dhara Water Quality Analysis System`,
            20,
            yPosition
         );
         yPosition += 6;
         pdf.text(
            `For more information, contact: support@aquabot.ai`,
            20,
            yPosition
         );
         yPosition += 6;
         pdf.text(
            `Page ${pdf.internal.pages.length - 1} of ${
         ...
         // Save the PDF
         pdf.save(
            `location-trends-${selectedLocation}-${new Date()
               .toISOString()
               .slice(0, 10)}.pdf`
         );
   ...
   const exportAllChartsPDF = async () => {
      ...
      try {
         toast({
            ...
            description:
               'Exporting all charts (Concentrations, Indices, Distribution, Trends)...',
         });
         ...
         pdf.setFont('helvetica', 'bold');
         pdf.text(
            'Groundwater Quality Analysis Report',
            pageWidth / 2,
            yPosition,
            { align: 'center' }
         );
         ...
         if (overviewMetrics) {
            ...
            pdf.setFont('helvetica', 'normal');
            pdf.text(
               `Average HPI: ${overviewMetrics.avgHpi}`,
               margin,
               yPosition
            );
            yPosition += 7;
            pdf.text(
               `High Pollution Samples: ${overviewMetrics.highPollutionCount}`,
               margin,
               yPosition
            );
            yPosition += 7;
            pdf.text(
               `Compliance Rate: ${overviewMetrics.complianceRate}%`,
               margin,
               yPosition
            );
            yPosition += 7;
            pdf.text(
               `Trend: ${overviewMetrics.trend} (${overviewMetrics.trendPercentage}%)`,
               margin,
               yPosition
            );
            yPosition += 7;
            pdf.text(
               `Most Polluted Metal: ${overviewMetrics.mostPollutedMetal}`,
               margin,
               yPosition
            );
         }
         ...
         const captureChartToPDF = async (
            ...
            if (chartElement) {
               try {
                  ...
                  pdf.setFontSize(10);
                  pdf.text(
                     `[Chart could not be rendered: ${title}]`,
                     margin,
                     yPosition
                  );
               }
      ...
      `;
                ...
                ${concentrationData
                   .map((metal, index) => {
                      ...
                      const maxValue = Math.max(
                         ...concentrationData.map((d) => d.maximum)
                      );
                    ...
                    <text x="${
                       x + barWidth / 2
                    }" y="340" text-anchor="middle" font-size="10">${metal.name.substring(
                         0,
                         3
                      )}</text>
                    <text x="${x + barWidth / 2}" y="${
                         y - 5
                      }" text-anchor="middle" font-size="8">${metal.average.toFixed(
                         3
                      )}</text>
                  `;
                   })
        ...
        `;
            ...
            await captureChartToPDF(
               'temp-concentrations-chart',
               'Metal Concentrations',
               'Average concentrations of heavy metals across all samples'
            );
              ...
              <svg width="100%" height="100%" viewBox="0 0 800 400">
                ...
                ${indicesData
                   ...
                   .map((sample, index) => {
                      ...
                      const maxValue = Math.max(
                         ...indicesData.reduce(
                            (acc, s) => [...acc, s.HPI, s.HEI, s.CD, s.NPI],
                            []
                         )
                      );
                    ...
                    <rect x="${x}" y="${
                         320 - hpiHeight
                      }" width="${barWidth}" height="${hpiHeight}" fill="#a855f7"/>
                    <rect x="${x + barWidth + 2}" y="${
                         320 - heiHeight
                      }" width="${barWidth}" height="${heiHeight}" fill="#f97316"/>
                    <rect x="${x + (barWidth + 2) * 2}" y="${
                         320 - cdHeight
                      }" width="${barWidth}" height="${cdHeight}" fill="#06b6d4"/>
                    <rect x="${x + (barWidth + 2) * 3}" y="${
                         320 - npiHeight
                      }" width="${barWidth}" height="${npiHeight}" fill="#ec4899"/>
                    <text x="${
                       x + groupWidth / 2
                    }" y="340" text-anchor="middle" font-size="8">${sample.name.substring(
                         0,
                         6
                      )}</text>
                  `;
        ...
        `;
            ...
            await captureChartToPDF(
               ...
               'Comparison of HPI, HEI, CD, and NPI across samples'
            );
          ...
          <div id="temp-distribution-chart" style="height: 400px; width: 100%;">
            <div style="height: 100%; width: 100%;">
              <svg width="100%" height="100%" viewBox="0 0 800 400">
                ...
                ${hpiDistribution
                   .map((item, index) => {
                      const total = hpiDistribution.reduce(
                         (sum, d) => sum + d.value,
                         0
                      );
                      const percentage = (item.value / total) * 100;
                      const angle = (percentage / 100) * 360;
                      const startAngle = hpiDistribution
                         .slice(0, index)
                      ...
                      const x2 =
                         200 + 80 * Math.cos(((endAngle - 90) * Math.PI) / 180);
                      const y2 =
                         180 + 80 * Math.sin(((endAngle - 90) * Math.PI) / 180);
                    ...
                    <text x="${
                       ...
                       100 *
                          ...
                          y="${
                             ...
                             100 *
                                Math.sin(
                                   ((startAngle + angle / 2 - 90) * Math.PI) /
                                      180
                                )
                          }" 
                          text-anchor="middle" font-size="10" fill="white">${percentage.toFixed(
                             0
                          )}%</text>
                  `;
                   })
                ...
                ${heiDistribution
                   .map((item, index) => {
                      const total = heiDistribution.reduce(
                         (sum, d) => sum + d.value,
                         0
                      );
                      const percentage = (item.value / total) * 100;
                      const angle = (percentage / 100) * 360;
                      const startAngle = heiDistribution
                         .slice(0, index)
                         .reduce((sum, d) => sum + (d.value / total) * 360, 0);
                      const endAngle = startAngle + angle;
                      ...
                      const y1 =
                         ...
                         80 * Math.sin(((startAngle - 90) * Math.PI) / 180);
                      const x2 =
                         600 + 80 * Math.cos(((endAngle - 90) * Math.PI) / 180);
                      const y2 =
                         180 + 80 * Math.sin(((endAngle - 90) * Math.PI) / 180);
                ...
                <!-- Legend -->
                ${[...new Set([...hpiDistribution, ...heiDistribution])]
                   .map(
                      (item, index) => `
                  <rect x="300" y="280 + ${
                     index * 20
                  }" width="15" height="15" fill="${item.color}"/>
                  <text x="320" y="292 + ${index * 20}" font-size="10">${
                         item.name
                      }: ${item.value}</text>
                `
                   )
        ...
        `;
            ...
            await captureChartToPDF(
               ...
               'Distribution of water quality categories for HPI and HEI'
            );
            ...
            <div style="height: 100%; width: 100%;">
              <svg width="100%" height="100%" viewBox="0 0 800 400">
                ...
                ${[0, 1, 2, 3, 4]
                   .map(
                      (i) => `
                  <line x1="100" y1="${80 + i * 60}" x2="720" y2="${
                         80 + i * 60
        ...
        `;
            ...
            await captureChartToPDF(
               ...
               'Temporal trends of pollution indices across all samples'
            );
         } finally {
         ...
         concentrationData.forEach((metal) => {
            pdf.text(
               `${metal.name}: Avg=${metal.average}, Max=${metal.maximum}, Min=${metal.minimum}`,
               margin + 10,
               yPosition
            );
            yPosition += 7;
         ...
         if (hpiValues.length > 0) {
            const hpiAvg =
               hpiValues.reduce((sum, val) => sum + val, 0) / hpiValues.length;
            pdf.text(
               `HPI: Average=${hpiAvg.toFixed(2)}, Range=${Math.min(
                  ...hpiValues
               ).toFixed(2)}-${Math.max(...hpiValues).toFixed(2)}`,
               margin + 10,
               yPosition
            );
            yPosition += 7;
         ...
         if (heiValues.length > 0) {
            const heiAvg =
               heiValues.reduce((sum, val) => sum + val, 0) / heiValues.length;
            pdf.text(
               `HEI: Average=${heiAvg.toFixed(2)}, Range=${Math.min(
                  ...heiValues
               ).toFixed(2)}-${Math.max(...heiValues).toFixed(2)}`,
               margin + 10,
               yPosition
            );
            yPosition += 7;
         ...
         if (cdValues.length > 0) {
            const cdAvg =
               cdValues.reduce((sum, val) => sum + val, 0) / cdValues.length;
            pdf.text(
               `CD: Average=${cdAvg.toFixed(2)}, Range=${Math.min(
                  ...cdValues
               ).toFixed(2)}-${Math.max(...cdValues).toFixed(2)}`,
               margin + 10,
               yPosition
            );
            yPosition += 7;
         ...
         if (npiValues.length > 0) {
            const npiAvg =
               npiValues.reduce((sum, val) => sum + val, 0) / npiValues.length;
            pdf.text(
               `NPI: Average=${npiAvg.toFixed(2)}, Range=${Math.min(
                  ...npiValues
               ).toFixed(2)}-${Math.max(...npiValues).toFixed(2)}`,
               margin + 10,
               yPosition
            );
            yPosition += 7;
         ...
         // Save the PDF
         const fileName = `groundwater-charts-${
      ...
      <div className="space-y-6">
         ...
         <div
            ...
            <p
               ...
               {new Date().toLocaleTimeString()} | Total Samples:{' '}
               {samples.length}
            </p>
            ...
            <div
               ...
               <p
                  ...
                  Average, maximum, and minimum concentrations of heavy metals
                  (mg/L)
               </p>
            ...
            <div
               ...
               <p
                  ...
                  Distribution of water quality categories across different
                  indices
               </p>
               <div
                  style={{
            ...
            <div id="chart4" style={{ marginBottom: '40px' }}>
               ...
               <p
                  ...
                  Pollution indices trends over time across all samples
                  (chronological order)
               </p>
               <div style={{ height: '300px', width: '100%' }}>
                  <ResponsiveContainer width="100%" height="100%">
            ...
            <div
               ...
               <div
                  ...
                  <p>
                     <strong>Metal Concentrations Summary:</strong>
                  </p>
                  {concentrationData.map((metal) => (
                     <p key={metal.name} style={{ marginLeft: '20px' }}>
                        {metal.name}: Average={metal.average} mg/L, Maximum=
                        {metal.maximum} mg/L, Minimum={metal.minimum} mg/L
                     </p>
                  ))}

                  <p style={{ marginTop: '15px' }}>
                     <strong>Pollution Indices Summary:</strong>
                  </p>
                  {(() => {
                     const hpiValues = indicesData
                        .map((d) => d.HPI)
                        .filter((v) => v > 0);
                     const heiValues = indicesData
                        .map((d) => d.HEI)
                        .filter((v) => v > 0);
                     const cdValues = indicesData
                        .map((d) => d.CD)
                        .filter((v) => v > 0);
                     const npiValues = indicesData
                        .map((d) => d.NPI)
                        .filter((v) => v > 0);
                        ...
                        <div style={{ marginLeft: '20px' }}>
                           {hpiValues.length > 0 && (
                              <p>
                                 ...
                                 , Range={Math.min(...hpiValues).toFixed(2)}-
                                 {Math.max(...hpiValues).toFixed(2)}
                              </p>
                           )}
                           {heiValues.length > 0 && (
                              <p>
                                 ...
                                 , Range={Math.min(...heiValues).toFixed(2)}-
                                 {Math.max(...heiValues).toFixed(2)}
                              </p>
                           )}
                           {cdValues.length > 0 && (
                              <p>
                                 ...
                                 , Range={Math.min(...cdValues).toFixed(2)}-
                                 {Math.max(...cdValues).toFixed(2)}
                              </p>
                           )}
                           {npiValues.length > 0 && (
                              <p>
                                 ...
                                 , Range={Math.min(...npiValues).toFixed(2)}-
                                 {Math.max(...npiValues).toFixed(2)}
                              </p>
                           )}
                        </div>
                     );
         ...
         <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
               ...
               <p className="text-sm sm:text-base text-muted-foreground">
                  Charts and analytics for groundwater pollution data
               </p>
            </div>
                           ...
                           <div className="flex items-center justify-between">
                              <div>
                                 ...
                                 <p className="text-2xl font-bold text-green-800">
                                    {overviewMetrics.complianceRate}%
                                 </p>
                              </div>
                              <CheckCircle className="h-8 w-8 text-green-500" />
                           </div>
                        ...
                        <CardContent className="p-4">
                           <div className="flex items-center gap-2 mb-2">
                              ...
                              <p className="text-sm font-medium text-orange-700">
                                 Critical Locations (
                                 {overviewMetrics.highPollutionCount} samples)
                              </p>
                           </div>
                           <div className="flex flex-wrap gap-2">
                              {overviewMetrics.criticalLocations.length > 0 ? (
                                 overviewMetrics.criticalLocations.map(
         ...
         <Tabs
            ...
            <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-1 h-auto p-1">
               ...
               <TabsTrigger
                  value="trends"
                  className="text-xs sm:text-sm py-2 px-2 hidden sm:block">
                  Overall Trends
               </TabsTrigger>
               <TabsTrigger
                  value="location-trends"
                  className="text-xs sm:text-sm py-2 px-2 hidden lg:block">
                  Location Trends
               </TabsTrigger>
               {/* Mobile fallback for hidden tabs */}
               <TabsTrigger
                  value="trends"
                  className="text-xs sm:text-sm py-2 px-2 sm:hidden">
                  Trends
               </TabsTrigger>
               <TabsTrigger
                  value="location-trends"
                  className="text-xs sm:text-sm py-2 px-2 lg:hidden">
                  Location
               </TabsTrigger>
            </TabsList>
               ...
               <motion.div
                  ...
                  <Card id="concentrations-chart">
                     <CardHeader>
                        ...
                        <CardDescription>
                           Average, maximum, and minimum concentrations of heavy
                           metals (mg/L)
                        </CardDescription>
                     </CardHeader>
               ...
               <motion.div
                  ...
                  <Card id="indices-chart">
                     <CardHeader>
                        ...
                        <CardDescription>
                           Comparison of different pollution indices across
                           samples
                        </CardDescription>
                     </CardHeader>
                     <CardContent className="p-2 sm:p-4">
                     ...
                     <motion.div
                        ...
                        <Card>
                           <CardHeader>
                              ...
                              <CardDescription>
                                 Heavy Metal Pollution Index categories
                              </CardDescription>
                           </CardHeader>
                           <CardContent className="p-2 sm:p-4">
                     ...
                     <motion.div
                        ...
                        <Card>
                           <CardHeader>
                              ...
                              <CardDescription>
                                 Heavy Metal Evaluation Index categories
                              </CardDescription>
                           </CardHeader>
                           <CardContent className="p-2 sm:p-4">
                              <ResponsiveContainer width="100%" height={250}>
                     ...
                     <motion.div
                        ...
                        <Card>
                           <CardHeader>
                              ...
                              <CardDescription>
                                 Contamination Degree categories
                              </CardDescription>
                           </CardHeader>
                           <CardContent className="p-2 sm:p-4">
                              <ResponsiveContainer width="100%" height={250}>
                     ...
                     <motion.div
                        ...
                        <Card>
                           <CardHeader>
                              ...
                              <CardDescription>
                                 Nemerow Pollution Index categories
                              </CardDescription>
                           </CardHeader>
                           <CardContent className="p-2 sm:p-4">
                              <ResponsiveContainer width="100%" height={250}>
                                    ...
                                    if (active && payload && payload.length) {
                                       ...
                                       return (
                                          <div className="bg-white p-2 sm:p-3 border rounded-lg shadow-lg max-w-xs">
                                             <p className="font-semibold mb-1 sm:mb-2 text-sm">
                                                Sample #{label}
                                             </p>
               ...
               <Card>
                  <CardHeader className="pb-4 sm:pb-6">
                     ...
                     <CardDescription className="text-sm sm:text-base">
                        Track pollution trends for specific locations over time.
                        Perfect for monitoring repeated sampling at the same
                        location.
                     </CardDescription>
                  </CardHeader>
                     ...
                     {samples.length === 0 ? (
                        <div className="text-center py-8 sm:py-12 bg-amber-50 rounded-lg px-4">
                           ...
                           <p className="text-amber-600 mb-4 text-sm sm:text-base">
                              Please add some sample data first to see location
                              trends.
                           </p>
                           <p className="text-xs sm:text-sm text-amber-500">
                              Go to the main page and add sample data using the
                              Data Input tab.
                           </p>
                        </div>
                        ...
                        <div className="text-center py-8 sm:py-12 bg-blue-50 rounded-lg px-4">
                           ...
                           <p className="text-blue-600 text-sm sm:text-base">
                              Please wait while we process the location data...
                           </p>
                        </div>
                     ) : (
                        <>
                                    ...
                                    <div className="flex items-center justify-between">
                                       <div>
                                          <p className="text-xs sm:text-sm text-green-600 font-medium">
                                             Locations with Trends
                                          </p>
                                          <p className="text-xl sm:text-2xl font-bold text-green-800">
                                             {
                                                getUniqueLocations().filter(
                                                   (loc) => loc.count > 1
                                                ).length
                                             }
                                          </p>
                           ...
                           <div className="space-y-4">
                              ...
                              {locationTrendData.length > 0 && (
                                 <div className="space-y-2">
                                    <label className="text-sm font-medium">
                                       Select Pollution Indices to Display:
                                    </label>
                                    <div className="flex flex-wrap gap-1 sm:gap-2">
                                       {[
                                          {
                                    ...
                                    <div className="space-y-2">
                                       ...
                                       {showBenchmark && (
                                          <div className="text-xs text-gray-500 mt-1">
                                             💡 Only shows benchmarks for metals
                                             approaching limits
                                          </div>
                                       )}
                                    </div>
                              ...
                              {!trendLoading && !selectedLocation && (
                                 <div className="text-center py-12 bg-gray-50 rounded-lg">
                                    ...
                                    <p className="text-gray-500">
                                       Choose a location from the dropdown to
                                       view trend analysis.
                                    </p>
                                 </div>
                              )}
                                    ...
                                    <div className="space-y-4 sm:space-y-6">
                                                   ...
                                                   <LineChart
                                                      ...
                                                      {customTimeRange && (
                                                         <Brush
                                                            ...
                                                            height={30}
                                                            stroke="#8884d8"
                                                            startIndex={getTrendChartData().findIndex(
                                                               (item) =>
                                                                  item.date >=
                                                                  new Date(
                                                                     timeRange.start
                                                            ...
                                                            endIndex={getTrendChartData().findIndex(
                                                               (item) =>
                                                                  item.date >=
                                                                  new Date(
                                                                     timeRange.end
                                       ...
                                       <Card>
                                          <CardHeader className="pb-4 sm:pb-6">
                                             <CardTitle className="flex items-center gap-2 text-lg sm:text-xl">
                                                <TrendingUp className="h-5 w-5" />
                                                Pollution Indices Trends for{' '}
                                                {selectedLocation}
                                             </CardTitle>
                                             <CardDescription className="text-sm sm:text-base">
                                                Pollution index changes over
                                                time for {selectedLocation}
                                             </CardDescription>
                                          </CardHeader>
                                          <CardContent
                                             id="pollution-index-chart"
                                             className="p-2 sm:p-4">
                                             <ResponsiveContainer
                                                width="100%"
                                                height={450}>
                                       ...
                                       <Card>
                                          <CardHeader className="pb-4 sm:pb-6">
                                             ...
                                             <CardDescription className="text-sm sm:text-base">
                                                Detailed sample data for{' '}
                                                {selectedLocation} sorted by
                                                date
                                             </CardDescription>
                                          </CardHeader>
                                          <CardContent className="p-2 sm:p-4">
